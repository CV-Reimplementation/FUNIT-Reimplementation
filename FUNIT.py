...
class FUNIT(object) :
    ...
    def build_model(self):
        ...
        self.train_loader = DataLoader(self.train_folder, batch_size=self.batch_size, shuffle=True, drop_last=True, num_workers=self.num_workers)
        self.test_folder = CustomDataset(os.path.join(self.dataset, 'test'), test_transform, target_num=self.K)
        self.test_loader = DataLoader(self.test_folder, batch_size=self.batch_size, shuffle=False, drop_last=False, num_workers=self.num_workers)
    ...
    def test(self):
        ...
        os.makedirs(os.path.join(self.result_dir, self.name, 'MRI'), exist_ok=True)
        ...
        with torch.no_grad():
            for tar, tar_label in test_iter:
                ...
                con_A = self.ConEn_(tar)
                cls_code = torch.zeros(0, self.batch_size, self.code_dim).to(self.device)

                for i in range(train.shape[1] // 3):
                    cls = self.ClsEn_(train[:, 3 * i:3 * (i + 1)])
                    cls_code = torch.cat((cls_code, cls.unsqueeze(0)), 0)

                cls_code = torch.mean(cls_code, 0)
                result = self.Dec_(con_A, cls_code)

                output = RGB2BGR(tensor2numpy(denorm(result[0])))

                if tar_label == 0:
                    cv2.imwrite(os.path.join(self.result_dir, self.name, 'CT', f_name[0]), output * 255.0)
                else:
                    cv2.imwrite(os.path.join(self.result_dir, self.name, 'MRI', f_name[0]), output * 255.0)
